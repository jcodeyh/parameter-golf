...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,bigram.scale,loop_scale",
    ).split(",")
    if pattern
)
...
class CausalSelfAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, rope_base: float, qk_gain_init: float):
        ...
        self.use_enable_gqa = (num_kv_heads != num_heads) and tuple(int(x) for x in torch.__version__.split(".")[:2]) >= (2, 2)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.use_enable_gqa:
            y = F.scaled_dot_product_attention(
                q, k, v, attn_mask=None, is_causal=True,
                enable_gqa=True,
            )
        elif self.num_kv_heads != self.num_heads:
            # Manually repeat KV heads to match Q head count for older PyTorch without enable_gqa
...
class GPT(nn.Module):
    ...
    def _init_weights(self) -> None:
        ...
        effective_depth = len(self.blocks) * self.loop_count
    ...
    def _run_blocks(self, x: Tensor, x0: Tensor) -> Tensor:
        for loop_idx in range(self.loop_count):
            ...
            if self.loop_scales is not None and loop_idx < self.loop_count - 1:
                x = F.rms_norm(x, (x.size(-1),))
                x = x * self.loop_scales[loop_idx].to(dtype=x.dtype)[None, None, :]
        return x
...
def main() -> None:
    ...
    effective_matrix_lr = args.matrix_lr / math.sqrt(args.loop_count)
    optimizer_muon = Muon(
        matrix_params,
        lr=effective_matrix_lr,
        momentum=args.muon_momentum,
        backend_steps=args.muon_backend_steps,
        weight_decay=0.04,
    )
    for group in optimizer_muon.param_groups:
        group["base_lr"] = effective_matrix_lr
    ...
    effective_depth = args.num_layers * args.loop_count
    log0(f"model_params:{n_params} unique_layers:{args.num_layers} loop_count:{args.loop_count} effective_depth:{effective_depth}")
    log0(f"world_size:{world_size} grad_accum_steps:{grad_accum_steps}")
    log0(f"attention_mode:gqa num_heads:{args.num_heads} num_kv_heads:{args.num_kv_heads}")
    log0(
        f"tie_embeddings:{args.tie_embeddings} embed_lr:{token_lr} "
        f"matrix_lr:{effective_matrix_lr} (base:{args.matrix_lr}/sqrt({args.loop_count})) scalar_lr:{args.scalar_lr}"
    )
    log0(
